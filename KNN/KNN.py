...
def decode_idx3_ubyte(idx3_ubyte_file):
    """解析idx3文件的通用函数"""
    print(f"Trying to open: {idx3_ubyte_file}")
...
def decode_idx1_ubyte(idx1_ubyte_file):
    """解析idx1文件的通用函数"""
    print(f"Trying to open: {idx1_ubyte_file}")
...
def main():
    ...
    visualize_predictions(X_test, y_test, y_pred, y_proba)


def visualize_predictions(X_test, y_test, y_pred, y_proba, num_images=8):
    # 随机选择 num_images 个索引
    ...
    for i, ax in enumerate(axes):
        ...
        confidence = np.max(y_proba[index])
