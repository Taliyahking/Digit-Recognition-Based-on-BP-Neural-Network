...
class NueraLNet(object):
    ...
    def predict(self, image_array):
        output = self.get_result(image_array)  # 调用前向传播函数获取结果
        return np.argmax(output)  # 返回最高概率的类别索引
...
def prepare_image(image_path):
    ...
    img = img.resize((28, 28), Image.LANCZOS)  # 调整为28x28像素
    img_array = np.array(img, dtype=np.float32)
    img_array /= 255.0  # 归一化
    img_array = img_array.reshape(1, 784)  # 重塑为1x784向量
    return img_array
